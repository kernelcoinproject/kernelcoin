...
TxType MWEB::GetTxType(const std::vector<CRecipient>& recipients, const std::set<CInputCoin>& input_coins)
{
    ...
    static auto is_kcn = [](const CInputCoin& input) { return !input.IsMWEB(); };
    ...
    if (recipients.front().IsMWEB()) {
        ...
        if (std::any_of(input_coins.cbegin(), input_coins.cend(), is_kcn)) {
            return TxType::PEGIN;
        } else {
        ...
        // Otherwise, if there are no MWEB inputs, it's a simple KCN-to-KCN transaction.
        if (std::any_of(input_coins.cbegin(), input_coins.cend(), is_mweb)) {
            return TxType::PEGOUT;
        } else {
            return TxType::KCN_TO_KCN;
        }
    }
}
...
uint64_t MWEB::CalcMWEBWeight(const MWEB::TxType& mweb_type, const bool change_on_mweb, const std::vector<CRecipient>& recipients)
{
    ...
    if (mweb_type != MWEB::TxType::KCN_TO_KCN) {
...
void Transact::AddMWEBTx(InProcessTx& new_tx)
{
    ...
    // Lookup the change paid on the KCN side
    CAmount kcn_change = 0;
    if (new_tx.change_position != -1) {
        assert(new_tx.tx.vout.size() > (size_t)new_tx.change_position);
        kcn_change = new_tx.tx.vout[new_tx.change_position].nValue;
    }

    // Calculate pegin_amount
    boost::optional<CAmount> pegin_amount = boost::none;
    CAmount kcn_input_amount = std::accumulate(
    ...
    if (kcn_input_amount > 0) {
        const CAmount kcn_fee = new_tx.total_fee - new_tx.mweb_fee;
        assert(kcn_fee <= kcn_input_amount);
        pegin_amount = (kcn_input_amount - (kcn_fee + kcn_change));
    }

    // Add Change
    if (new_tx.change_on_mweb) {
        receivers.push_back(BuildChangeRecipient(new_tx, pegin_amount, kcn_change));
    }
...
mw::Recipient Transact::BuildChangeRecipient(const InProcessTx& new_tx, const boost::optional<CAmount>& pegin_amount, const CAmount& kcn_change)
{
    ...
    CAmount change_amount = (pegin_amount.value_or(0) + mweb_input_amount) - (recipient_amount + new_tx.mweb_fee + kcn_change);
    if (change_amount < 0) {
