...
const char * const BITCOIN_CONF_FILENAME = "kernelcoin.conf";
...
#else
    const char* pszModule = "kernelcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Kernelcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Kernelcoin";
#else
    // Unix-like
    return pathRet / ".kernelcoin";
#endif
#endif
}
