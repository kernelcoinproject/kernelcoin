Kernelcoin Core version *0.15.1* is now available from:

  <https://download.kernelcoin.org/kernelcoin-0.15.1/>
...
A number of changes to the way Kernelcoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 cobs per kB (0.00001 KCN). This is to prepare for dropping the minimum transaction fee to 0.1 cobs per kB (0.0001 KCN) in the next release.
