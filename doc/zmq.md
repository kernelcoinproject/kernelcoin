...
The Kernelcoin Core daemon can be configured to act as a trusted "border
router", implementing the kernelcoin wire protocol and relay, making
...
The ZeroMQ feature in Kernelcoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building kernelcoind:
...
For instance:
    ...
    $ kernelcoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/kernelcoind.tx.raw \
...
These options can also be provided in kernelcoin.conf.
...
From the perspective of kernelcoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into kernelcoind directly. Furthermore, no information is
...
using. Kernelcoind appends an up-counting sequence number to each
